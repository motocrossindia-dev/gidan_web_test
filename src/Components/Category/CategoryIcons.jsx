...
const CategoryIcons = () => {
  ...
  const getCategorywiseProduct = (id, categoryname, slug, typeKey) => {
    // Special categories with custom routes
    if (categoryname === "GIFTS") {
      ...
      // Navigate using clean slug-based URLs WITH state data
      navigate(`/${slug}/`, {
        state: {
          categoryId: id,
  ...
  return (
      <>
        <div className="max-w-7xl mx-auto px-2 sm:px-4 lg:px-5 relative z-10">
          {/* Category Container - removed overflow-y-visible as it conflicts with overflow-x-auto */}
          <div
              id="category-scroll-container"
              className="flex items-center gap-3 sm:gap-4 md:gap-6 lg:gap-8 px-2 sm:px-4 mt-4 sm:py-2 w-full overflow-x-auto whitespace-nowrap scrollbar-hide md:flex-wrap md:justify-between md:overflow-visible"
              style={{ minHeight: '115px' }}
          >
            {publishedCategoryData.map((category, idx) => (
                <div
                    key={idx}
                    className="relative shrink-0 flex flex-col items-center mb-2 min-w-0 text-center group static md:relative"
                >
                  <div className="flex flex-col items-center">
                    ...
                    >
                      <img
                          ...
                          loading="lazy" width="200" height="200" style={{ aspectRatio: '1/1' }}
                      />
                    </div>
                  ...
                  {/* Dropdown positioned with fixed on mobile, absolute on desktop */}
                  <div className="fixed md:absolute top-[140px] md:top-full left-4 md:left-0 mt-0 md:mt-2 w-[calc(100vw-2rem)] max-w-[280px] md:w-[180px] md:max-w-[220px] bg-white border border-gray-300 shadow-xl rounded-lg z-[9999] origin-top-left opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200">
