      ...
      <div className="max-w-7xl mx-auto px-2 sm:px-4 lg:px-5">
        ...
        >
          {publishedCategoryData.map((category, idx) => (
            <div
              key={idx}
              className="relative group shrink-0 flex flex-col items-center mb-2 min-w-0 text-center"
            >
              <Link
                to={
                  category.name === "GIFTS"
              ...
              >
                <div className="w-12 h-12 xs:w-14 xs:h-14 sm:w-16 sm:h-16 md:w-18 md:h-18 lg:w-20 lg:h-20 xl:w-22 xl:h-22 border-2 border-gray-400 hover:border-gray-500 rounded-full flex items-center justify-center bg-white shadow-md overflow-hidden transition-all duration-200 hover:shadow-lg">
                  <img
                    src={`${process.env.REACT_APP_API_URL}${category.image}`}
                    alt={category.name || "Category"}
                    onClick={() =>
                      getCategorywiseProduct(category.id, category.name)
                    }
              ...
              <div className="hidden md:group-hover:block absolute top-full left-0 mt-2 w-[180px] sm:w-[200px] md:w-[220px] bg-white border border-gray-200 shadow-lg rounded-lg z-[999] origin-top-left">
